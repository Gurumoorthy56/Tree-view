...
document.getElementById("chip-input").addEventListener("keydown", function (e) {
    if (e.key === 'ArrowDown') {
        ...
        if (currentFocus > -1 && itemsList[currentFocus]) {
            ...
            // let pendingChildInputs;

            allLI.forEach(li => {
                if (li.innerText == itemsList[currentFocus].innerText) {
                    // const childInputs = li.querySelectorAll('input')
                    // pendingChildInputs = countUnSelected(childInputs);
                    // if (pendingChildInputs + chipList.length <= 30) {
                    checkBox(itemsList[currentFocus].innerText);
                    // console.log('sjjks')
                    // console.log(`${childInputs} --> ${chipList.length}`)
                    ...
                    // }
                    return true;
                    // allInputs.forEach(input => { if (input.checked == false) { li.parentElement.querySelector('input').checked = false; console.log('sasa') } })
                }
...
function showParents(parentLi) {
    // console.log(parentLi.querySelector('text'))
    parentLi = parentLi.parentElement;

    while (parentLi.tagName === 'UL') {
        parentLi.style.display = 'block';
        parentLi.parentElement.querySelector('.material-icons').textContent = 'arrow_drop_down';
